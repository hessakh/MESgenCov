...
\section{MESgenCov implementation}
 ...
 The \textbf{MESgenCov} package contains functions in the \texttt{S3} class to create a covariance matrix from the desired subset of NADP data. The function \verb;getCov(); returns a covariance matrix, a list of univariate model summaries, and a table of normality tests produced internally by the MVN R package. The covariance matrix is produced from a subset of the NADP NTN data that is specified by the user. For sites with missing data, \verb;getCov(); will fill in predicted values based on the univariate model for each site. To avoid sites with a small sample size for the specified time-frame, the function \verb;getSites(); outputs a vector of the sites with the largest sample of data for a given time-frame and measured chemical. To find sites that are spatially ``spread out'' but have at least some specified sample size, the function \verb;maxDistSites(); can be used.
  ...
  \verb;getCov(); is by far the most complicated function, and it has 19 inputs that allow the user specify the subset of data to analyze and give the user options in displying different parts of the analysis. A table of the input is given here:
...
<<chunckCovMult,include=TRUE,eval=FALSE>>=
  getCov(FALSE,"01/01/83 00:00", "12/31/89 00:00",TRUE,NULL,NULL,NULL,
...
<<chunckCovSite,include=TRUE,eval=FALSE>>=
  getCov(FALSE,"01/01/83 00:00", "12/31/86 00:00",TRUE,c("OH71","NY08"),NULL,NULL,
         FALSE,NULL,"SO4",FALSE,TRUE,"IL11",FALSE,FALSE,12,1,3,1)
@
%%
...
\section{Methodology}
 ...
 For a given monitoring site, chemical, and month $t=0,1,\ldots,T-1$, let
      \begin{align*}
        ...
        q_w  &:= \text{recorded chemical quantity for week $w$ ($c(w)=*$ denotes an unrecorded value),} \\
        p_d  &:= \text{recorded precipitation quantity for day $d$.}
    \end{align*}
Then the chemical's concentration for month $t$ is
\[
c(t):=\frac{\sum_{w\in W(t)} q_w}
{\sum_{w\in W(t) : q_w \not=*} \sum_{d\in w} p_d}.
\]
Finally, a model is fit to $\log(c(t))$.

    It should be noted that when there is no weekly value available for the chemical quantity,
    we do not add the preciptation values for any of the days in such a week
    (so as to not artificially dilute the chemical concentration level for the month).  In **citep** they use the following model to deseasonalize and detrend the data set of log transformed monthly sulfate concentration values.

\begin{equation} \label{simplemodel}
         \log(c(t))\approx \beta_{1} + \beta_{2}t + \beta_{3}\cos\bigg(\frac{2\pi t}{12}\bigg) + \beta_{4}\sin\bigg(\frac{2\pi t}{12}\bigg)~.
\end{equation}
  ...
  certain cites but some sites like "MD13" and "NC03" did not do as well. Instead, we fit a more flexible model described in the next section.%also show figures for sites
...
\[
\log(c(t))\approx \sum_{i=0}^r \beta_{i}t^i + \sum_{j=1}^{k} \bigg[a_j \cos \bigg(\frac{2\pi jt}{S}\bigg) +  b_j \sin \bigg(\frac{2\pi jt}{S}\bigg)\bigg].
\]
The user can specify the degree $r$ for the polynomial part of the model, aimed at capturing aperiodic trends.
Periodic trends can be captured via a truncated Fourier series, truncated at level $k$.
