...
#'beta version with optional inputs
#'@import MVN
#'@import rmatio
#'@import EnvStats
...
#'@export
#'@examples
#'   getCovmissing(FALSE,"01/01/83 00:00","12/31/86 00:00",
#use36 = TRUE, showOutliers = FALSE, comp = c("SO4"),
#plotMulti = FALSE,plotB = FALSE,sitePlot = NULL,plotAll = FALSE, 
#writeMat = FALSE,seas = 12,r = 1,k = 3)


getCovmissing <- function(weeklyB,startdateStr,enddateStr,
  ...
  if (missing(outlierDatesbySite)){
    outlierDatesbySite <- NULL
  }
  if (missing(showOutliers)){
    showOutliers <- FALSE
  }
  if (missing(siteOutliers)){
    siteOutliers <- NULL
  }
  if(missing(plotMulti)){
    plotMulti = FALSE
  }
  if(missing(plotB)){
    plotB = FALSE
