...
\section{Methodology}
  ...
  We process the data in an identical way to earlier attempts to use the data to solve an instance of the MES problem in the field of environmental statistics. \cite{Zidek3}
 analyzes the levels of sulfate concentrations by adding up weekly values of mg/L of sulfate and dividing the monthly total by total precipitation in that month, to get monthly values of sulfate concentration \color{blue}(unit).\color{black}
      \begin{align*}
