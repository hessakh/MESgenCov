...
\title{MESgenCov: An R Package for creating covariance matrices from NADP monitor data}
\author{Hessa Al-Thani$^1$ and Jon Lee\\[0.35cm]
  \small{University of Michigan, Department of Industrial, Operations Engineering, Ann Arbor, MI}\\[0cm]\textbf{\small{$^1$hessa.k.h9@gmail.com}}
}
\date{\textbf{MESgenCov} version \Sexpr{packageDescription("MESgenCov")$Version}  }
%
...
\begin{abstract}
  We present an R package for fitting multivariate climate data and extracting an entropy model. Our approach does not fit a spatial model, but rather uses the maximum-entropy sampling (MES) approach instead. We allow for multiple functionalities for modeling and model assessment. The package uses data from the NADP/NTN (National Atmospheric Deposition Program / National Trends Network) on their set of more than 370 monitoring sites, 1978-present. The user specifies the sites, pollutants and time period desired, fits a user-controlled univariate model, and produces a covariance matrix for use by MES algorithms.

\end{abstract}

\section{MES problem description}

The maximum-entropy sampling problem has been applied to many domains where the objective is to decide on the most informative subset $S$ from a set of $N$ variables. This is done by maximizing the log determinant of the covariance matrix for some $S \subseteq N$. In this package we use the data gethered by the NADP on acidic deposits at sites across the US. For this instance of the MES problem the sites make up the n-dimensional covariance matrix.
%%add a bit on generalized getCov after finishing the function

\section{NADP data description}

The National Atmospheric Deposition Program (NADP) maintains the National Trends Network (NTN), this network measures deposits in precipitation at sites across the United States. Our package makes use of the weekly data measuring the mg/L of pollutants like sulfate in sampled rainfall. We also use the daily data measuring precipitation at each site. Both datasets are available in the package and can be loaded respectively as
...
\section{MESgenCov implementation}
 ...
 The \textbf{MESgenCov} package contains functions in the \texttt{S3} class to create the covariance matrix from the desired subset of NADP data. The function getCov() returns a covariance matrix, a list of univariate model summaries, and a table of normality tests produced internally by the MVN R package. The covariance matrix is produced from a subset of the NADP NTN data that is sepecified by the user. For sites with missing data getCov will fill in predicted values based on the univariate model for each site. To avoid sites with a small sample size for the specified time-frame the function getSites outputs a vector of the sites with the largest sample of data for a given time-frame and measured pollutant. To find sites that are far-away from eachother but have at least some specified sample size the function maxDistSites() can be used.

  \subsection{getCov}

  getCov is by far the largest function and it has 19 inputs that allow the user specify the subset of data to analyze and give the user options in displying different parts of the analysis. A table of the input is given here:
...
%%%%Parsing results
<<chunkCovResultuni,include=TRUE,eval=TRUE>>=
  result <- getCov(FALSE,"01/01/83 00:00", "12/31/86 00:00",FALSE,
                   c("OH71", "NY08", "WV18", "MI53", "NH02"),c(1,2,3,4),NULL,
                   FALSE,NULL,"SO4",FALSE,FALSE,NULL,FALSE,FALSE,12,1,3,1)
