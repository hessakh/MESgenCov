...
getCovmissing1 <- function(df){
  ...
  for(s in 1:(length(cati)*length(obs))){
    ...
    if(nrow(conCSVk)==0 || nrow(preCSVk)==0){
      ...
      cn <- colnames(sitem)
      #sitem <- na.omit(sitem) figure out how to make this work for weekly data to make sure NA is ignored in min and plot vectors are the same size
      if(weeklyB){
        minsite3 <- min(sitem[,3])
      }else{
        minsite3 <- 0
      }
  ...
  par(mar=c(1,1,1,1))
  MVDw <- mvn(dfRes[,2:10], subset = NULL, mvnTest = "mardia", covariance = TRUE, tol = 1e-25, alpha = 0.5, scale = FALSE, desc = TRUE, transform = "none", univariateTest = "SW",  univariatePlot = "histogram", multivariatePlot = "none", multivariateOutlierMethod = "none", bc = FALSE, bcType = "rounded", showOutliers = FALSE,showNewData = FALSE)
  univariateTest <- MVDw$univariateNormality
  MVDw
  if(plotMulti){
    dev.new(width = 8, height = 5, noRStudioGD = TRUE)
    par(mfrow=c(1,2))
    MVDw <- mvn(dfRes[,-1], subset = NULL, mvnTest = "mardia", covariance = TRUE, tol = 1e-25, alpha = 0.5, scale = FALSE, desc = TRUE, transform = "none", univariateTest = "SW",  univariatePlot = "histogram", multivariatePlot = "qq", multivariateOutlierMethod = "quan", bc = FALSE, bcType = "rounded", showOutliers = TRUE, showNewData = FALSE)
    MVDw
  }
  siteRosner = NULL
