...
\date{\textbf{MESgenCov} \Sexpr{packageDescription("MESgenCov")$Version}  }
%
...
%
    ...
    Output = c("cov", "listMod", "sites", "mvn","univariateTest", "residualData",  "residualDataNA", "rosnerTest", "pred","noutliers"),
    Definition = c( "Covariance matrix produced by univariate model residuals", "List of univariate model summaries produced by lm()", "List of sites that were analyzed", "Output of the MVN package","univariate test output, also by the MVN package", "Data frame of residuals produced by the univariate model",  "Data frame of residuals, where missing values are ledt as NA", "Output of the rosner test for outlier analysis produced by the EnvStats package", "List of predicted values produced by the univariate model for each site","Number of outliers detected by the outlier analysis, default is 0 if no sites are analyzed for outliers")
    )
 @
 ...
 library(magrittr)
 library(kableExtra)
     kable(text_tbl2, format = "latex", booktabs = T) %>%
     kable_styling(full_width = F) %>%
     column_spec(1, bold = T, color = "black") %>%
     column_spec(2, width = "30em") %>%
         kable_styling(latex_options = "striped")
...
<<chunckCuni,include=TRUE,eval=TRUE>>=
...
<<chunckOutSiteAll,include=TRUE,eval=TRUE>>=
...
<<chunkCovMat2,include=TRUE,eval=FALSE>>=
  library(rmatio)
...
@
 ...
 This function will produce a list of four elements that can be called using the following names:
  <<lambert2,include=TRUE,eval=TRUE>>=
  data("dfRes50")
  loutput <- lambertWtransform(dfRes = dfRes50, plotMulti = FALSE, writeMat = FALSE)
  loutput$mvn$multivariateNormality
...
<<chunckIndExample,include=TRUE,eval=TRUE>>=
#get list of sites
