...
function (bi,site,siObs,obs,obsi,totT,strtYrMo){#binary con or pH
  sitem <- data.frame(matrix(ncol=5, nrow=totT))
  yrm        <- site[1,5]
...
>>>>>>> 76088de28b86ff6d0029c53ddcc66bf371db51c6
  ...
  for (i in 0:(totT-2+mStrt)){# handles case with no precip data for month and skips in months
    ...
    if(sum(siteyrmonth[,obsi + 5+bi]) == 0){
      ...
      sitem[i+1,4] <- (sum(siteyrmonth[,obsi+5+bi]))
      sitem[i+1,5] <- (sum(siteyrmonth[,obsi+6+bi]))
  ...
  }else{colnames(sitem) <- c("yrmonth", "t", paste(obs[obsi],"Con",sep=""),"precipitation","Vol")}
  sitem
}
